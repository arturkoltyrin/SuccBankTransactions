...
def test_load_transactions_file_not_exist():
    ...
    with patch("os.path.exists", return_value=False), patch("os.path.isfile", return_value=False):
...
def test_load_transactions_empty_file():
    ...
    with patch("builtins.open", mock_open(read_data="")) as mock_file, patch(
...
def test_load_transactions_valid_json():
    ...
    with patch("builtins.open", mock_open(read_data=json.dumps(transactions))) as mock_file, patch(
        "os.path.exists", return_value=True
    ), patch("os.path.isfile", return_value=True):
        result = load_transactions("fake_path.json")
...
def test_load_transactions_invalid_json():
    ...
    with patch("builtins.open", mock_open(read_data="invalid json")) as mock_file, patch(
        "os.path.exists", return_value=True
    ), patch("os.path.isfile", return_value=True):
        result = load_transactions("fake_path.json")
        assert result == []
