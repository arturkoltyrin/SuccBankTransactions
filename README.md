...
### Модуль `test_read_financ_transactions`
Функции написаны с использованием библиотеки `pandas`.
#### Функции:

**`load_transactions_from_csv(filepath: str) -> List[Dict[str, str]]`**

Функция загружает финансовые транзакции из csv-файла.
...
Параметры:
- filepath (str): Путь к excel-файлу с транзакциями.
...
**`main`**
Основная функция, отвечающая за логику работы программы. 
Функционал:
- Запрашивает у пользователя выбор формата файла для загрузки транзакций.
- Фильтрует транзакции по статусу.
- Позволяет конвертировать суммы в рубли.
- Позволяет фильтровать транзакции по описанию.
- Позволяет сортировать транзакции по дате.
...
### 13. `load_transactions_from_csv`
Функция считывает финансовые операции из CSV файла и возвращает список словарей.

#### Тесты:
- Входные данные: файл file.csv с корректными данными.
  - Ожидаемый результат: список из двух словарей с данными транзакций.
- При возникновении исключения (например, "файл не найден"):
  - Ожидаемый результат: исключение с сообщением "Ошибка при считывании файла CSV: File not found".

Функция возвращают список словарей, каждый из которых содержит информацию о транзакции.

### 14. `load_transactions_from_excel`
Функция считывает финансовые операции из Excel файла и возвращает список словарей.

#### Тесты:
- Входные данные: файл file.xlsx с корректными данными.
  - Ожидаемый результат: список из двух словарей с данными транзакций.
- При возникновении исключения (например, "файл не найден"):
  - Ожидаемый результат: исключение с сообщением "Ошибка при считывании файла Excel: File not found".
